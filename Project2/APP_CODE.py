...
#convert the input address to lambert-72 coordinates, so we can match it later with the right tif-files
...
#search for right tif file (code adjusted for testing)
#only one file path here because not all csv's are in the database or program folder
path = "./tile_12040.tif"
...
# plot 3D house with values from dsm & dtm tiff files
